...
for (var i = 0; i < favoritos.length; i++) {
  favoritos[i].addEventListener("click", function (event) {
    ...
    xhr.open(
      "POST",
      "index.php?controller=" + controller + "&action=" + action + "&id=" + id
    );
    ...
    xhr.onload = function () {
      if (this.status === 200) {
        ...
        if (action === "save") {
          ...
          elemento.innerHTML =
            '<img src="assets/img/logo_guardar_r.png" alt="Icono Bookmark guardado">';
        } else {
          elemento.setAttribute("isSaved", "false");
          elemento.innerHTML =
            '<img src="assets/img/logo_guardar_l.png" alt="Icono Bookmark no guardado">';
        }
      }
    };
    xhr.send();
