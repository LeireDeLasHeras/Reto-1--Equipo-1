...
document.addEventListener("DOMContentLoaded", function () {
  ...
  form.addEventListener("submit", function (event) {
    ...
    if (!nicknamePattern.test(nicknameInput.value)) {
      mostrarMensajeError(
        nicknameInput,
        "El nickname solo puede contener letras, números, y los símbolos especiales: _ , - ."
      );
      esValido = false;
    }
    ...
    if (!apellidoPattern.test(apellidoInput.value)) {
      mostrarMensajeError(
        apellidoInput,
        "Ingrese uno o dos apellidos. Cada apellido debe comenzar con mayúscula y contener solo letras."
      );
      esValido = false;
    }
    ...
    if (!correoPattern.test(correoInput.value)) {
      mostrarMensajeError(
        correoInput,
        "El correo debe terminar con @egibide.org."
      );
      esValido = false;
    }
    ...
    if (!passwordPattern.test(passwordInput.value)) {
      mostrarMensajeError(
        passwordInput,
        "La contraseña debe tener al menos 8 caracteres, con 1 número, 1 mayúscula y 1 caracter especial (. , - _)."
      );
      esValido = false;
    }
